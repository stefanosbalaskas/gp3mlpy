...
from gp3mlpy.objects import GP3MLConformal, GP3MLThresholdEvaluation


def test_decision_rule_construction_validation_and_probability_guards():
    assert np.allclose(dg._assert_probability([0.0, np.nan, 1.0])[[0, 2]], [0.0, 1.0])
    for bad in (["bad"], [np.inf], [-0.1]):
    ...
    rule = dg.create_gazepoint_decision_rule(
        ...
        scientific_justification="prespecified threshold",
    )
    assert dg.validate_gazepoint_decision_rule(rule, require_threshold=True).status == "pass"
    assert "0.5" in repr(rule)

    pending = dg.create_gazepoint_decision_rule(
        metric="balanced_accuracy",
        threshold=None,
        generalization_target="new_participants",
        scientific_justification="estimate only within training data",
    )
    assert "<not selected>" in repr(pending)
    assert dg.validate_gazepoint_decision_rule(pending).status == "pass"
    assert dg.validate_gazepoint_decision_rule(pending, require_threshold=True).status == "fail"
    ...
    malformed = rule.__deepcopy__({})
    malformed.direction = "bad"
    malformed.threshold = 2.0
    malformed.threshold_origin = "bad"
    malformed.training_partition = "assessment"
    malformed.generalization_target = ""
    malformed.scientific_justification = ""
    malformed.abstention_allowed = True
    malformed.abstention_interval = [0.8, 0.2]
    assert dg.validate_gazepoint_decision_rule(malformed, require_threshold=True).status == "fail"
    assert dg.validate_gazepoint_decision_rule(object()).status == "fail"


def test_threshold_selection_application_and_abstention_paths():
    ...
    selected = dg.select_gazepoint_threshold(
        ...
        generalization_target="new_participants",
        scientific_justification="selected on analysis data",
    )
    ...
    abstain_rule = dg.create_gazepoint_decision_rule(
        ...
        generalization_target="new_participants",
        scientific_justification="abstain near boundary",
    )
    ...
    audit = dg.audit_gazepoint_abstention(["control", "target", "target"], decision)
    assert audit.status == "pass" and audit.coverage == pytest.approx(2 / 3)
    all_abstain = dg.audit_gazepoint_abstention(["a", "b"], [".abstain", ".abstain"])
    assert all_abstain.status == "fail" and np.isnan(all_abstain.covered_error_rate)
    assert np.isnan(dg.audit_gazepoint_abstention([], []).coverage)
    with pytest.raises(GP3MLError, match="lengths differ"):
        dg.audit_gazepoint_abstention(["a"], [])

    invalid_rule = abstain_rule.__deepcopy__({})
    invalid_rule.threshold = None
    with pytest.raises(GP3MLError, match="validation failed"):
        dg.apply_gazepoint_decision_rule(invalid_rule, [0.5], "yes", "no")
    ...
    base = dict(truth=truth, probability=probability, positive="target", thresholds=[0.5])
    ...
    with pytest.raises(GP3MLError, match="threshold_origin"):
        dg.select_gazepoint_threshold(evaluation, "accuracy", threshold_origin="predeclared")
...
def test_conformal_fit_predict_validate_and_coverage_paths():
    ...
    assert len(cf.predict_gazepoint_interval(reg, [1.5, 2.5])) == 2
    ...
    assert len(cf.predict_gazepoint_set(cls, [0.05, 0.5, 0.95])) == 3

    manual = GP3MLConformal(
        task_type="classification",
        positive="target",
        negative="control",
        level=0.5,
    ...
    assert cf.predict_gazepoint_set(manual, [0.1, 0.5, 0.9]).set.tolist() == [
        "{control}", "{control, target}", "{target}"
    ]
    none = manual.__deepcopy__({})
    ...
    for changes, match in bad_reg_calls:
        args = dict(base)
        args.update(changes)
        with pytest.raises(GP3MLError, match=match):
            cf.fit_gazepoint_conformal(**args)
    with pytest.raises(GP3MLError, match="probability"):
        cf.fit_gazepoint_conformal(ctruth, probability=[0.1], task_type="classification")
    with pytest.raises(GP3MLError, match="exactly two"):
        cf.fit_gazepoint_conformal(
            ["a", "b", "c"], probability=[0.1, 0.5, 0.9], task_type="classification"
        )
    for unit in (None, ["p1", None, "p2", "p2"]):
    ...
    with pytest.raises(GP3MLError, match="No finite conformity"):
        cf.fit_gazepoint_conformal(
            [np.nan, np.nan], prediction=[np.nan, np.nan], task_type="regression"
        )
    with pytest.raises(GP3MLError, match="No finite conformity"):
        cf._type1_quantile(np.array([np.nan]), 0.5)

    assert cf.validate_gazepoint_conformal(object()).status == "fail"
    invalid = manual.__deepcopy__({})
    invalid.task_type = "bad"
    invalid.level = 2
    invalid.calibration_unit = "bad"
    invalid.conformity_quantile = np.nan
    invalid.generalization_target = ""
    invalid.n_calibration_units = 1
    ...
    reg_grouped = grouped.__deepcopy__({})
    ...
    assert coverage.status in {"pass", "review"}
    assert np.isfinite(coverage.row_coverage)
    with pytest.raises(GP3MLError, match="interval"):
        cf.assess_gazepoint_conformal_coverage(reg_grouped, truth, interval=None)
    with pytest.raises(GP3MLError, match="complete and aligned"):
        cf.assess_gazepoint_conformal_coverage(
            reg_grouped, truth, interval=interval, unit=["p1", None, "p2", "p2"]
        )
    ...
    cc = cf.assess_gazepoint_conformal_coverage(
        manual, ["control", "target", "control", None], set=sets
    )
    assert np.isfinite(cc.row_coverage)
    assert np.isnan(cc.unit_coverage)
    with pytest.raises(GP3MLError, match="classification `set`"):
        cf.assess_gazepoint_conformal_coverage(manual, ["control"], set=None)


def test_dataset_and_missingness_shift_severity_and_summary_paths():
    ...
    empty_dev = pd.DataFrame(
        {"num": pd.Series([], dtype=float), "cat": pd.Series([], dtype=object)}
    )
    empty = ds.audit_gazepoint_dataset_shift(empty_dev, empty_dev.copy())
    assert empty.status in {"pass", "review"}
    assert np.isnan(
        empty.findings.loc[empty.findings.predictor == "num", "outside_training_range"].iloc[0]
    )
    no_predictors = ds.audit_gazepoint_dataset_shift(
        development.iloc[:, :0], external_pass.iloc[:, :0]
    )
    assert no_predictors.status == "review" and no_predictors.findings.empty
    with pytest.raises(GP3MLError, match="absent"):
        ds.audit_gazepoint_dataset_shift(development, external_pass, ["nope"])

    miss_pass = ds.audit_gazepoint_missingness_shift(development, external_pass, ["missing"])
    assert miss_pass.status == "pass"
    ext_review = development.copy()
    ext_review["missing"] = [np.nan, 2.0, np.nan, 4.0]
    assert ds.audit_gazepoint_missingness_shift(
        development, ext_review, ["missing"], review_delta=0.2, fail_delta=0.8
    ).status == "review"
    ext_fail = development.copy()
    ext_fail["missing"] = np.nan
    miss_fail = ds.audit_gazepoint_missingness_shift(
        development, ext_fail, ["missing"], review_delta=0.1, fail_delta=0.5
    )
    assert miss_fail.status == "fail"
    assert ds.audit_gazepoint_missingness_shift(
        development.iloc[:, :0], external_pass.iloc[:, :0]
    ).status == "review"
    with pytest.raises(GP3MLError, match="absent"):
    ...
    assert "n_predictors" in ds._status_counts(failed.findings)
    ...
    assert "missingness_shift_status" not in ds.summarize_gazepoint_shift(passed)
