...
def fixture(target: str = "new_participants"):
    data = gp.simulate_gazepoint_governed_data(
        n_participants=18, n_stimuli=4, trials_per_cell=1, seed=17
    )
    task = gp.create_gazepoint_synthetic_task(
        data, workflow="recording_quality", generalization_target=target
    )
...
def test_synthetic_task_manifest_and_roles():
    ...
    assert gp.validate_gazepoint_feature_manifest(manifest).status == "pass"
...
def test_new_participant_split_is_disjoint():
    ...
    assert set(split.analysis[task.participant_id]).isdisjoint(
        set(split.assessment[task.participant_id])
    )
...
def test_leakage_audit_clean_and_invalid_predictor():
    ...
    bad = gp.audit_gazepoint_ml_leakage(
        split.analysis, split.assessment, task.outcome, [task.outcome]
    )
    assert bad.status == "fail"
...
def test_decision_rule_and_thresholds():
    ...
    rule = gp.create_gazepoint_decision_rule(
        metric="balanced_accuracy",
        threshold=0.5,
        scientific_justification="Predeclared observed-outcome classification threshold.",
        generalization_target="new_trials_known_participants",
    )
    assert gp.validate_gazepoint_decision_rule(rule).status == "pass"
    table = gp.evaluate_gazepoint_thresholds(
        truth, prob, thresholds=[0.3, 0.5, 0.7], positive="yes"
    )
    assert len(table.thresholds) == 3
    chosen = gp.select_gazepoint_threshold(
        table,
        metric="balanced_accuracy",
        direction="maximize",
        scientific_justification="Inner-resampling threshold selection for an observed outcome.",
        generalization_target="new_trials_known_participants",
    )
    classified = gp.apply_gazepoint_decision_rule(
        chosen, prob, positive="yes", negative="no"
    )
...
def test_conformal_regression():
    ...
    fit = gp.fit_gazepoint_conformal(
        truth,
        prediction=pred,
    ...
    coverage = gp.assess_gazepoint_conformal_coverage(
        fit, truth[:5], interval=intervals
    )
...
def test_dataset_shift_summary():
    ...
    ext = pd.DataFrame({"x": np.arange(20) + 2, "z": np.linspace(0.1, 1.1, 20)})
...
def test_locked_analysis_plan_is_immutable():
    plan = gp.declare_gazepoint_analysis_plan(
        ...
        outcome_definition="observed binary label",
...
def test_release_checksum_contract(tmp_path: Path):
    ...
    artifact.write_text("gp3mlpy\n")
    manifest = gp.write_gazepoint_release_checksums(
        [artifact], tmp_path / "SHA256SUMS.csv"
    )
    assert gp.validate_gazepoint_release_checksums(
        manifest, directory=tmp_path
    ).status == "pass"


def test_handoff_and_governance_profile():
    data, task, manifest = fixture()
    handoff = gp.create_gazepoint_handoff(
        data,
        source_package="gp3tools",
        keys=[task.participant_id, task.stimulus_id, task.unit_id],
        outcome=task.outcome,
        predictors=PREDICTORS,
        feature_manifest=manifest,
    )
    assert gp.validate_gazepoint_handoff(handoff).status == "pass"
    profile = gp.create_gp3ml_governance_profile(
        {"purpose_declared": True, "prohibited_use_screened": True}
    )
...
def test_group_fold_evaluation_smoke():
    ...
    result = gp.evaluate_gazepoint_group_folds(
        folds, task, PREDICTORS, engine="glm", seed=17
    )
    assert gp.validate_gazepoint_resample_evaluation(result).status == "pass"
    assert len(gp.collect_gazepoint_fold_predictions(result)) > 0
    assert len(gp.summarize_gazepoint_resample_performance(result).summary) > 0
