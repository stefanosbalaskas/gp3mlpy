...
def _data_manifest_task():
    data = gp.simulate_gazepoint_governed_data(
        n_participants=12, n_stimuli=4, trials_per_cell=1, seed=7
    )
    ...
    task = gp.declare_gazepoint_task(
        ...
        purpose="assigned-condition prediction",
...
def test_decision_rule_requires_scientific_justification():
    with pytest.raises(GP3MLError):
        gp.create_gazepoint_decision_rule(
            metric="youden", generalization_target="new_participants"
        )
...
def test_locked_analysis_plan_is_immutable():
    plan = gp.declare_gazepoint_analysis_plan(
        research_question="Q",
        scientific_purpose="prediction",
        outcome="outcome",
        outcome_definition="binary assigned condition",
        predictors=["x1", "x2"],
        generalization_target="new_participants",
        grouping_variables=["participant_id"],
        eligible_population="synthetic participants",
        preprocessing_plan="fold local",
        candidate_models=["glm"],
        primary_metric="roc_auc",
        secondary_metrics=["brier"],
        calibration_metric="brier",
        uncertainty_method="participant bootstrap",
        threshold_policy="predeclared",
        seed_strategy="fixed",
    )
