...
def _compare(left: Any, right: Any, *, atol: float, rtol: float, path: str = "root") -> list[str]:
    errors: list[str] = []
...
def main() -> int:
    if len(sys.argv) != 5:
        raise SystemExit(
            "Usage: python parity/compare_core.py <fixture.json> <r-output.json> <python-output.json> <report.json>"
        )
    ...
    prohibited_errors = _compare(
        r_result.get("prohibited_uses"),
        python_result.get("prohibited_uses"),
        atol=atol,
        rtol=rtol,
        path="gp3ml_prohibited_uses",
    )
    cases.append(
        {
            "function": "gp3ml_prohibited_uses",
            "case_id": "prohibited_uses_exact",
            "status": "PASS" if not prohibited_errors else "FAIL",
            "errors": prohibited_errors,
        }
    )

    for group, function_name in (
        ("classification", "gazepoint_classification_metrics"),
        ("regression", "gazepoint_regression_metrics"),
    ):
    ...
    print(f"core parity: {report['overall_status']}")
    for case in cases:
