...
def main() -> int:
    if len(sys.argv) != 5:
        raise SystemExit("Usage: python parity/compare_leakage.py <fixture.json> <r.json> <python.json> <report.json>")
    ...
    for group, function_name in (("audits", "audit_gazepoint_ml_leakage"), ("writers", "write_gazepoint_ml_leakage_audit_csv")):
        r_cases = r_result.get(group, {})
        py_cases = py_result.get(group, {})
        for case_id in sorted(set(r_cases) | set(py_cases)):
            errors = _compare(r_cases.get(case_id), py_cases.get(case_id), atol=atol, rtol=rtol, path=f"{function_name}.{case_id}")
            cases.append({"function": function_name, "case_id": case_id, "status": "PASS" if not errors else "FAIL", "errors": errors})
    ...
    print(f"leakage parity: {report['overall_status']}")
    for case in cases:
