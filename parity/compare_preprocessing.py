...
STATUS_ONLY = {
    "fit_too_few_rows_error",
    "fit_invalid_numeric_imputation",
    "fit_invalid_novel_level",
}
...
def _compare(left: Any, right: Any, *, atol: float, rtol: float, path: str) -> list[str]:
    ...
    if isinstance(left, dict) and isinstance(right, dict):
        ...
        for key in sorted(set(left) & set(right)):
            errors.extend(
                _compare(
                    left[key], right[key], atol=atol, rtol=rtol, path=f"{path}.{key}"
                )
            )
        return errors
...
def main() -> int:
    if len(sys.argv) != 5:
        raise SystemExit(
            "Usage: python parity/compare_preprocessing.py "
    ...
    groups = (
        ("fits", "fit_gazepoint_preprocessor"),
        ("fit_errors", "fit_gazepoint_preprocessor"),
        ("bakes", "bake_gazepoint_preprocessor"),
        ("bake_errors", "bake_gazepoint_preprocessor"),
    )
    ...
    print(f"preprocessing parity: {report['overall_status']}")
    for case in cases:
