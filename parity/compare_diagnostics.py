...
def main() -> int:
    if len(sys.argv) != 5:
        raise SystemExit(
            "Usage: python parity/compare_diagnostics.py "
    ...
    groups = (
        ("successes", "diagnose_gazepoint_group_folds"),
        ("errors", "diagnose_gazepoint_group_folds"),
        ("validations", "validate_gazepoint_fold_diagnostics"),
        ("writers", "write_gazepoint_fold_diagnostics_csv"),
    )
    ...
    print(f"diagnostics parity: {report['overall_status']}")
    for case in cases:
