...
ERROR_KEYWORDS = {
    ("create_gazepoint_nested_folds", "invalid_outer"): ["gazepoint_group_folds"],
    ("create_gazepoint_nested_folds", "invalid_inner_v"): ["inner_v"],
    ("create_gazepoint_nested_folds", "invalid_inner_repeats"): ["inner_repeats"],
    ("audit_gazepoint_nested_resampling", "invalid_object"): ["gp3ml_nested_folds"],
    ("validate_gazepoint_nested_folds", "invalid_object"): ["gp3ml_nested_folds"],
    ("evaluate_gazepoint_nested_resampling", "invalid_nested"): ["gp3ml_nested_folds"],
    ("evaluate_gazepoint_nested_resampling", "invalid_grid"): ["gp3ml_tuning_grid"],
    ("evaluate_gazepoint_nested_resampling", "stop_on_outer_failure"): ["outer fold", "failed"],
    ("validate_gazepoint_nested_evaluation", "invalid_object"): ["gp3ml_nested_evaluation"],
    ("write_gazepoint_nested_evaluation", "overwrite_rejected"): ["overwrite"],
    ("write_gazepoint_nested_evaluation", "invalid_object"): ["gp3ml_nested_evaluation"],
}
    ...
    for runtime, case in (("R", r_case), ("Python", py_case)):
        ...
        for keyword in ERROR_KEYWORDS.get((function, case_id), []):
            if keyword.lower() not in message:
                errors.append(
                    f"{path}: {runtime} error lacks token {keyword!r}: {message!r}"
                )
    return errors
...
def main() -> int:
    if len(sys.argv) != 6:
        raise SystemExit(
            "Usage: python parity/compare_nested_resampling.py "
        ...
        if not errors:
            if comparison == "error_contract":
                ...
                if r_case.get("status") != "success" or py_case.get("status") != "success":
                    errors.append(
                        f"{key}: expected success; "
                        f"R={r_case.get('status')!r}, Python={py_case.get('status')!r}"
                    )
                else:
                    errors.extend(
                        _compare(
