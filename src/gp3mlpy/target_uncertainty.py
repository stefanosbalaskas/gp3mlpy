...
def _cluster_bootstrap_indices(ids: Sequence[Any], rng: np.random.RandomState) -> np.ndarray:
    ...
    clusters = list(dict.fromkeys(str(x) for x in arr))
    sampled = rng.choice(clusters, size=len(clusters), replace=True)
    return np.concatenate([np.flatnonzero(np.asarray([str(x)==value for x in arr])) for value in sampled])
...
def _two_way_indices(participant_id, stimulus_id, rng, max_attempts=100):
    ...
    participants=list(dict.fromkeys(str(x) for x in p)); stimuli=list(dict.fromkeys(str(x) for x in s))
    ps=np.asarray([str(x) for x in p]); ss=np.asarray([str(x) for x in s])
    for _ in range(max_attempts):
        pdrawing=rng.choice(participants,size=len(participants),replace=True); sdrawing=rng.choice(stimuli,size=len(stimuli),replace=True)
        pc={x:int(np.sum(pdrawing==x)) for x in participants}; sc={x:int(np.sum(sdrawing==x)) for x in stimuli}
        weights=np.asarray([pc.get(a,0)*sc.get(b,0) for a,b in zip(ps,ss,strict=True)],dtype=int)
        idx=np.repeat(np.arange(len(weights)),weights)
...
def bootstrap_gazepoint_metrics_by_unit(
    task,
    truth,
    prediction=None,
    probability=None,
    ...
    seed: int=1,
    threshold: float=.5,
    stratify_observations: bool=True,
):
    ...
    pred_arr=None if prediction is None else np.asarray(prediction,dtype=object)
    ...
    rng=np.random.RandomState(int(seed)); draws=[]; failures=[]; replicate_sizes=[]
    for i in range(1,bootstrap+1):
        ...
        try:
            draw=gazepoint_performance_metrics(task,truth_arr[idx],None if pred_arr is None else pred_arr[idx],None if prob_arr is None else prob_arr[idx],threshold).copy()
            draw["replicate"]=i; draw["resample_n"]=len(idx); draws.append(draw)
        except Exception as exc:
            failures.append({"replicate":i,"error":str(exc)})
    if not draws: raise GP3MLError("Every bootstrap replicate failed.")
    draws_df=pd.concat(draws,ignore_index=True); failures_df=pd.DataFrame(failures,columns=["replicate","error"])
    metrics=[c for c in draws_df.columns if pd.api.types.is_numeric_dtype(draws_df[c]) and c not in {"n","threshold","replicate","resample_n"}]
    alpha=(1-conf_level)/2
    intervals=pd.DataFrame([{"metric":m,"estimate":float(point[m].iloc[0]),"lower":float(draws_df[m].quantile(alpha)),"upper":float(draws_df[m].quantile(1-alpha)),"successful_replicates":int(np.isfinite(draws_df[m]).sum())} for m in metrics])
    limitations={
...
def summarize_gazepoint_resample_uncertainty(evaluation, unit: str="fold", conf_level: float=.95):
    ...
    if unit=="repeat": distribution=metrics.groupby(["repeat","metric"],as_index=False)["value"].mean()
    else: distribution=metrics[["repeat","fold","fold_id","metric","value"]].copy()
    alpha=(1-conf_level)/2; rows=[]
...
def validate_gazepoint_target_uncertainty(x):
    ...
    statuses=["pass" if getattr(x,"unit",None) else "fail","pass" if getattr(x,"generalization_target",None) else "fail","pass" if getattr(x,"limitations",None) else "fail","pass" if (not is_target or getattr(x,"failed_replicates",None) is not None) else "fail"]
    checks=pd.DataFrame({"check_id":["unit_recorded","target_recorded","limitations_recorded","failed_replicates_recorded"],"status":statuses,"message":[f"Resampling unit: {getattr(x,'unit',None)}",f"Generalization target: {getattr(x,'generalization_target',None)}",str(getattr(x,'limitations',None)),f"Failed replicates: {x.failed_replicates}" if is_target else "Not a bootstrap object."]})
    return GP3MLUncertaintyValidation(status=worst_status(statuses),checks=checks,issues=checks[checks.status!="pass"].reset_index(drop=True))
...
def write_gazepoint_target_uncertainty(x,directory,prefix="gazepoint_target_uncertainty",overwrite=False):
    ...
    if x.r_class=="gp3ml_target_uncertainty": tables={"point":x.point,"intervals":x.intervals,"draws":x.draws,"failures":x.failures,"validation":validation.checks}
    else: tables={"summary":x.summary,"distribution":x.distribution,"validation":validation.checks}
    return write_tables(tables,directory,prefix,overwrite)
