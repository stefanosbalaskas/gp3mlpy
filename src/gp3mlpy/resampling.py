...
def _integer(x:Any,argument:str,minimum:int=1)->list[int]:
    ...
    if vals is None or not vals or any(isinstance(v,bool) or not isinstance(v,(int,np.integer)) or int(v)<minimum for v in vals):raise GP3MLError(f"`{argument}` must contain finite integers greater than or equal to {minimum}.")
...
def _assign_groups(values:np.ndarray,v:int,rng:np.random.Generator)->tuple[np.ndarray,pd.DataFrame]:
    ...
    sizes=np.array([(values==g).sum() for g in groups],dtype=int);random_ties=rng.permutation(np.arange(1,len(groups)+1));order=np.lexsort((random_ties,-sizes));fold_rows=np.zeros(v,dtype=int);fold_groups=np.zeros(v,dtype=int);group_fold=np.zeros(len(groups),dtype=int)
    for gp in order:
        candidates=np.flatnonzero(fold_rows==fold_rows.min());candidates=candidates[fold_groups[candidates]==fold_groups[candidates].min()];selected=int(candidates[0] if len(candidates)==1 else rng.choice(candidates));group_fold[gp]=selected+1;fold_rows[selected]+=sizes[gp];fold_groups[selected]+=1
    mapping=pd.DataFrame({"group":groups,"fold":group_fold,"n_rows":sizes});lookup=dict(zip(groups,group_fold,strict=True));row_fold=np.array([lookup[v] for v in values],dtype=int);return row_fold,mapping
...
def create_gazepoint_group_folds(data:pd.DataFrame,outcome:str,predictors:list[str]|tuple[str,...]|str,feature_manifest:pd.DataFrame,generalization_target:str,participant_id:str|None=None,trial_id:str|None=None,stimulus_id:str|None=None,v:Any=5,repeats:int=1,seed:int=1,source_row_id:str=".gp3ml_source_row")->GazepointGroupFolds:
    ...
    repeats=repeats_i[0];seed=seed_i[0];ok={"new_trials_known_participants":participant_id is not None and trial_id is not None,"new_participants":participant_id is not None,"new_stimuli":stimulus_id is not None,"new_participants_and_new_stimuli":participant_id is not None and stimulus_id is not None}[generalization_target]
    ...
    manifest,mvalid=_manifest_for_predictors(feature_manifest,pred);participant=_group_values(data,participant_id,"participant_id") if participant_id else None;trial=_group_values(data,trial_id,"trial_id") if trial_id else None;stimulus=_group_values(data,stimulus_id,"stimulus_id") if stimulus_id else None
    ...
    rng=np.random.default_rng(seed);source_rows=np.arange(1,len(data)+1,dtype=int);fold_data=data.copy();fold_data[source_row_id]=source_rows;folds={};assignments=[];summaries=[];counts_rows=[];mappings=[];fpr=vv["participant"]*vv["stimulus"] if generalization_target=="new_participants_and_new_stimuli" else vv["group"]
    for ri in range(1,repeats+1):
        ...
        if generalization_target=="new_participants_and_new_stimuli":
            for sf in range(1,vv["stimulus"]+1):
                for pf in range(1,vv["participant"]+1):specs.append((pf,sf))
        else:specs=list(range(1,vv["group"]+1))
        for fi,spec in enumerate(specs,start=1):
            if generalization_target=="new_participants_and_new_stimuli":hp,hs=spec;ph=participant_fold==hp;sh=stimulus_fold==hs;partition=np.where(ph&sh,"assessment",np.where(~ph&~sh,"analysis","excluded"));heldp=hp;helds=hs
            else:held=int(spec);partition=np.where(group_fold==held,"assessment","analysis");heldp=held if generalization_target=="new_participants" else None;helds=held if generalization_target=="new_stimuli" else None
            ...
            a=fold_data.loc[partition=="analysis"].reset_index(drop=True);e=fold_data.loc[partition=="assessment"].reset_index(drop=True);ex=fold_data.loc[partition=="excluded"].reset_index(drop=True);leak=audit_gazepoint_ml_leakage(a,e,outcome,pred,participant_id,trial_id,stimulus_id,generalization_target);folds[fid]=GazepointGroupFold(**{"repeat":ri,"fold":fi,"fold_id":fid,"participant_fold":heldp,"stimulus_fold":helds,"analysis":a,"assessment":e,"excluded":ex,"analysis_indices":aidx.tolist(),"assessment_indices":eidx.tolist(),"excluded_indices":xidx.tolist(),"leakage_audit":leak});assignments.append(pd.DataFrame({"repeat":ri,"fold":fi,"fold_id":fid,"source_row":source_rows,"partition":partition,"split_unit":split_unit,"group_fold":group_fold,"participant_fold":participant_fold,"stimulus_fold":stimulus_fold}));summaries.append({"repeat":ri,"fold":fi,"fold_id":fid,"participant_fold":heldp,"stimulus_fold":helds,"n_total":len(data),"n_analysis":len(aidx),"n_assessment":len(eidx),"n_excluded":len(xidx),"assessment_prop_all":len(eidx)/len(data),"assessment_prop_retained":len(eidx)/(len(aidx)+len(eidx)),"leakage_status":leak.status});c=_group_counts(data,partition,participant_id,trial_id,stimulus_id);c.insert(0,"repeat",ri);c.insert(1,"fold",fi);c.insert(2,"fold_id",fid);counts_rows.append(c[["repeat","fold","fold_id","partition","unit","n_groups"]])
...
def audit_gazepoint_group_folds(x:GazepointGroupFolds)->GazepointGroupFoldsAudit:
    ...
    for fo in x.folds.values():
        ...
        summary.append({"repeat":fo["repeat"],"fold":fo.fold,"fold_id":fo.fold_id,"status":audit.status,"n_pass":int((audit.checks.status=="pass").sum()),"n_review":int((audit.checks.status=="review").sum()),"n_fail":int((audit.checks.status=="fail").sum())});c=audit.checks.copy();c.insert(0,"repeat",fo["repeat"]);c.insert(1,"fold",fo.fold);c.insert(2,"fold_id",fo.fold_id);checkrows.append(c)
    s=pd.DataFrame(summary);checks=pd.concat(checkrows,ignore_index=True);issues=checks.loc[checks.status!="pass"].reset_index(drop=True);status="fail" if (s.status=="fail").any() else "review" if (s.status=="review").any() else "pass";return GazepointGroupFoldsAudit(status=status,summary=s,checks=checks,issues=issues)
...
def validate_gazepoint_group_folds(x:GazepointGroupFolds)->GazepointGroupFoldsValidation:
    ...
    add("materialized_partitions_match_assignments","pass" if material else "fail","Materialized partitions match the assignment table." if material else "Materialized partitions and assignments disagree.","None." if material else "Recreate the fold object.");ms=x.feature_manifest_validation.status;add("feature_manifest_passed",ms,f"Feature-manifest validation status is `{ms}`.","None." if ms=="pass" else "Resolve feature-provenance issues.");au=x.audit.status;add("fold_leakage_audit",au,f"Aggregated fold leakage-audit status is `{au}`.","None." if au=="pass" else "Review the fold-level leakage-audit issues.");c=pd.DataFrame(checks);issues=c.loc[c.status!="pass"].reset_index(drop=True);status="fail" if (c.status=="fail").any() else "review" if (c.status=="review").any() else "pass";summary=pd.DataFrame({"status":["pass","review","fail"],"n_checks":[int((c.status==s).sum()) for s in ["pass","review","fail"]]});return GazepointGroupFoldsValidation(status=status,summary=summary,checks=c,issues=issues,assessment_coverage=coverage)

def write_gazepoint_group_folds_csv(x:GazepointGroupFolds,directory:str|Path,prefix:str="gazepoint_group_folds",tables:Any=("assignments","fold_summary","group_counts","group_mapping","validation_checks","validation_issues","audit_summary","audit_checks","audit_issues"),include_fold_data:bool=False,overwrite:bool=False,na:str="")->dict[str,str]:
    if not isinstance(x,GazepointGroupFolds):raise GP3MLError("`x` must be a `gazepoint_group_folds` object.")
    ...
    data={"assignments":x.assignments,"fold_summary":x.fold_summary,"group_counts":x.group_counts,"group_mapping":x.group_mapping,"validation_checks":x.validation.checks,"validation_issues":x.validation.issues,"audit_summary":x.audit.summary,"audit_checks":x.audit.checks,"audit_issues":x.audit.issues};req=[tables] if isinstance(tables,str) else list(tables)
