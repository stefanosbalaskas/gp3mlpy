...
def gazepoint_classification_metrics(
    ...
    log_loss = float(-np.mean(y[valid] * np.log(p[valid]) + (1 - y[valid]) * np.log(1 - p[valid]))) if np.any(valid) else float("nan")
    return pd.DataFrame([{
        "n": int(np.sum(valid)), "threshold": threshold,
        "accuracy": _safe(tp + tn, tp + tn + fp + fn), "balanced_accuracy": balanced_accuracy,
        "sensitivity": sensitivity, "specificity": specificity, "precision": precision,
        "recall": sensitivity, "f1": f1,
        "mcc": float("nan") if mcc_den == 0 else float((tp * tn - fp * fn) / mcc_den),
        "roc_auc": _auc(y, p), "pr_auc": _pr_auc(y, p), "brier": brier, "log_loss": log_loss,
    }])
...
def gazepoint_performance_metrics(task: GP3MLTask, truth: Sequence[Any] | pd.Series, prediction: Sequence[Any] | pd.Series | None = None, probability: Sequence[float] | np.ndarray | None = None, threshold: float = 0.5) -> pd.DataFrame:
...
def bootstrap_gazepoint_metrics(task: GP3MLTask, truth: Sequence[Any] | pd.Series, prediction: Sequence[Any] | pd.Series | None = None, probability: Sequence[float] | np.ndarray | None = None, threshold: float = 0.5, bootstrap: int = 1000, conf_level: float = 0.95, seed: int = 1) -> GP3MLMetricUncertainty:
    ...
    if task.task_type == "classification":
        ...
        for _ in range(bootstrap):
            ...
            draws.append(gazepoint_classification_metrics(truth_arr[idx], prob_arr[idx], None if pred_arr is None else pred_arr[idx], task.positive, threshold))
    else:
    ...
    intervals = pd.DataFrame([{
        "metric": name, "estimate": float(point[name].iloc[0]),
    ...
    return GP3MLMetricUncertainty(point=point, intervals=intervals, draws=draws_df, bootstrap=bootstrap, conf_level=conf_level, seed=seed, task=task)
