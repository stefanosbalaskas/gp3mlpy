...
def _group_values(data:pd.DataFrame,column:str,argument:str)->np.ndarray:
    s=data[column];vals=s.astype("string").str.strip()
...
def _holdout_count(n_groups:int,proportion:float)->int:
    ...
    count=int(round(n_groups*proportion));return max(1,min(n_groups-1,count))
...
def _manifest_for_predictors(feature_manifest:pd.DataFrame|None,predictors:list[str])->tuple[pd.DataFrame,Any]:
    ...
    manifest=feature_manifest.set_index("feature",drop=False).loc[predictors].reset_index(drop=True);manifest.attrs.update(feature_manifest.attrs);validation=validate_gazepoint_feature_manifest(manifest)
...
def _two_way_assignment(participant:np.ndarray,stimulus:np.ndarray,assessment_prop:float,rng:np.random.Generator,max_attempts:int=250)->dict[str,Any]:
    participants=np.array(sorted(set(participant)));stimuli=np.array(sorted(set(stimulus)));pc=_holdout_count(len(participants),math.sqrt(assessment_prop));sc=_holdout_count(len(stimuli),math.sqrt(assessment_prop));best=None;best_score=float("inf")
    for _ in range(max_attempts):
        hp=np.sort(rng.choice(participants,size=pc,replace=False));hs=np.sort(rng.choice(stimuli,size=sc,replace=False));ph=np.isin(participant,hp);sh=np.isin(stimulus,hs);partition=np.where(ph&sh,"assessment",np.where(~ph&~sh,"analysis","excluded"))
        if not np.any(partition=="analysis") or not np.any(partition=="assessment"):continue
        score=abs(np.mean(partition=="assessment")-assessment_prop)+np.mean(partition=="excluded")
        if score<best_score:best_score=score;best={"partition":partition,"participant_held":ph,"stimulus_held":sh,"held_participants":hp.tolist(),"held_stimuli":hs.tolist()}
...
def _group_counts(data:pd.DataFrame,partition:np.ndarray,participant_id:str|None,trial_id:str|None,stimulus_id:str|None)->pd.DataFrame:
    ...
    if trial_id:
        trial=data[trial_id].astype(str).to_numpy();add("participant_trial",_trial_units(participant,trial)) if participant_id else add("trial",trial)
    if stimulus_id:add("stimulus",data[stimulus_id].astype(str).to_numpy())
    return pd.DataFrame(rows,columns=["partition","unit","n_groups"])

def split_gazepoint_ml_data(data:pd.DataFrame,outcome:str,predictors:list[str]|tuple[str,...]|str,feature_manifest:pd.DataFrame,generalization_target:str,participant_id:str|None=None,trial_id:str|None=None,stimulus_id:str|None=None,assessment_prop:float=0.20,seed:int=1,source_row_id:str=".gp3ml_source_row")->GazepointMLSplit:
    ...
    ids=[x for x in [participant_id,trial_id,stimulus_id,source_row_id] if x];idpred=[p for p in predictors_l if p in ids]
    ...
    required=[outcome,*predictors_l,*[x for x in [participant_id,trial_id,stimulus_id] if x]];missing=[c for c in dict.fromkeys(required) if c not in data.columns]
    if missing:raise GP3MLError("`data` is missing required columns: "+", ".join(missing)+".")
    manifest,manifest_validation=_manifest_for_predictors(feature_manifest,predictors_l);participant=_group_values(data,participant_id,"participant_id") if participant_id else None;trial=_group_values(data,trial_id,"trial_id") if trial_id else None;stimulus=_group_values(data,stimulus_id,"stimulus_id") if stimulus_id else None;rng=np.random.default_rng(int(seed))
    ...
    if generalization_target=="new_trials_known_participants":
        assert participant is not None and trial is not None;trial_units=_trial_units(participant,trial);split_unit=trial_units.copy();assessment_units=[]
        ...
        assert participant is not None;groups=np.array(sorted(set(participant)));held=rng.choice(groups,size=_holdout_count(len(groups),assessment_prop),replace=False);participant_held=np.isin(participant,held);split_unit=participant.copy();partition=np.where(participant_held,"assessment","analysis")
    elif generalization_target=="new_stimuli":
        assert stimulus is not None;groups=np.array(sorted(set(stimulus)));held=rng.choice(groups,size=_holdout_count(len(groups),assessment_prop),replace=False);stimulus_held=np.isin(stimulus,held);split_unit=stimulus.copy();partition=np.where(stimulus_held,"assessment","analysis")
    else:
        assert participant is not None and stimulus is not None;tw=_two_way_assignment(participant,stimulus,assessment_prop,rng);partition=tw["partition"];participant_held=tw["participant_held"];stimulus_held=tw["stimulus_held"];split_unit=np.array([f"{len(p)}:{p}|{len(s)}:{s}" for p,s in zip(participant,stimulus,strict=True)],dtype=object)
    ...
    source_rows=np.arange(1,n+1,dtype=int);split_data=data.copy();split_data[source_row_id]=source_rows;aidx=source_rows[partition=="analysis"];eidx=source_rows[partition=="assessment"];xidx=source_rows[partition=="excluded"];analysis=split_data.loc[partition=="analysis"].reset_index(drop=True);assessment=split_data.loc[partition=="assessment"].reset_index(drop=True);excluded=split_data.loc[partition=="excluded"].reset_index(drop=True)
    assignment=pd.DataFrame({"source_row":source_rows,"partition":partition,"split_unit":split_unit,"participant_held_out":participant_held,"stimulus_held_out":stimulus_held});retained=len(aidx)+len(eidx);summary=pd.DataFrame([{"generalization_target":generalization_target,"seed":int(seed),"assessment_prop_requested":float(assessment_prop),"assessment_prop_achieved_all":len(eidx)/n,"assessment_prop_achieved_retained":len(eidx)/retained,"n_total":n,"n_analysis":len(aidx),"n_assessment":len(eidx),"n_excluded":len(xidx)}]);groups=_group_counts(data,partition,participant_id,trial_id,stimulus_id);leakage=audit_gazepoint_ml_leakage(analysis,assessment,outcome,predictors_l,participant_id,trial_id,stimulus_id,generalization_target)
    result=GazepointMLSplit(analysis=analysis,assessment=assessment,excluded=excluded,analysis_indices=aidx.tolist(),assessment_indices=eidx.tolist(),excluded_indices=xidx.tolist(),assignment=assignment,summary=summary,group_counts=groups,feature_manifest=manifest,feature_manifest_validation=manifest_validation,leakage_audit=leakage,metadata={"outcome":outcome,"predictors":predictors_l,"participant_id":participant_id,"trial_id":trial_id,"stimulus_id":stimulus_id,"generalization_target":generalization_target,"assessment_prop":float(assessment_prop),"seed":int(seed),"source_row_id":source_row_id,"n_source_rows":n});result.validation=validate_gazepoint_ml_split(result);return result
...
def validate_gazepoint_ml_split(x:GazepointMLSplit)->GazepointMLSplitValidation:
    ...
    required=["analysis","assessment","excluded","assignment","summary","group_counts","feature_manifest_validation","leakage_audit","metadata"];missing=[c for c in required if c not in x]
    ...
    ar=x.analysis[sr].tolist();er=x.assessment[sr].tolist();xr=x.excluded[sr].tolist();add("analysis_non_empty","pass" if len(x.analysis)>0 else "fail","The analysis partition is non-empty." if len(x.analysis)>0 else "The analysis partition is empty.","None." if len(x.analysis)>0 else "Revise the split request.");add("assessment_non_empty","pass" if len(x.assessment)>0 else "fail","The assessment partition is non-empty." if len(x.assessment)>0 else "The assessment partition is empty.","None." if len(x.assessment)>0 else "Revise the split request.")
    ...
    complete=sorted(ar+er+xr)==list(range(1,int(x.metadata["n_source_rows"])+1));add("source_rows_fully_accounted","pass" if complete else "fail","All source rows are accounted for exactly once." if complete else "Source-row accounting is incomplete or invalid.","None." if complete else "Reconstruct the split from the original data.")
    ...
    ms=x.feature_manifest_validation.status;add("feature_manifest_passed",ms,f"Feature-manifest validation status is `{ms}`.","None." if ms=="pass" else "Resolve feature-provenance issues before evaluation.");ls=x.leakage_audit.status;add("leakage_audit_status",ls,f"Leakage-audit status is `{ls}`.","None." if ls=="pass" else "Review and resolve the embedded leakage-audit issues.")
    checks=pd.DataFrame(rows);issues=checks.loc[checks.status!="pass"].reset_index(drop=True);status="fail" if (checks.status=="fail").any() else "review" if (checks.status=="review").any() else "pass";summary=pd.DataFrame({"status":["pass","review","fail"],"n_checks":[int((checks.status==s).sum()) for s in ["pass","review","fail"]]});return GazepointMLSplitValidation(status=status,summary=summary,checks=checks,issues=issues,leakage_audit=x.leakage_audit,feature_manifest_validation=x.feature_manifest_validation)
...
def write_gazepoint_ml_split_csv(x:GazepointMLSplit,directory:str|Path,prefix:str="gazepoint_ml_split",tables:list[str]|tuple[str,...]|str=("analysis","assessment","excluded","assignment","summary","group_counts","checks","issues"),overwrite:bool=False,na:str="")->dict[str,str]:
    ...
    valid=["analysis","assessment","excluded","assignment","summary","group_counts","checks","issues"];requested=[tables] if isinstance(tables,str) else list(tables)
    if not requested or any(t not in valid for t in requested):raise GP3MLError("`tables` must use values from: "+", ".join(valid)+".")
    requested=list(dict.fromkeys(requested));d=Path(directory).expanduser();d.mkdir(parents=True,exist_ok=True);data={"analysis":x.analysis,"assessment":x.assessment,"excluded":x.excluded,"assignment":x.assignment,"summary":x.summary,"group_counts":x.group_counts,"checks":x.validation.checks,"issues":x.validation.issues};paths={t:d/f"{prefix}_{t}.csv" for t in requested};existing=[p for p in paths.values() if p.exists()]
